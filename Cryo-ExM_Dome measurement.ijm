...
// Macro to mesur the synaptic dome in T cells.
...
function DrawPerpendicularline(channel) {	
	...
	run("Reslice [/]...", "output=0.270 start=Top flip");
	rename("reslice");
	getDimensions(width, height, channels, slices, frames);
	n_slice = slices;
	
	
	range_y = (n_slice / 2) + 25;
//				print("Range_y: ", range_y);
	
	range_x = (n_slice / 2) - 25;
//				print("range_x: ", range_x);
	
	run("Z Project...", "start=range_x stop=range_y projection=[Max Intensity]");
...
for(roi=0; roi<lengthOf(roi_name); roi++){
	...
	if (roi_img_index >= 0 && roi_lng_index > roi_img_index) {
	    ...
	    sav_roi = "_Image ";
			...
			roi_img_index = indexOf(roiset, "serie");
			print("Fallback roi_img_index: ", roi_img_index);
			sav_roi = "_serie";
			
			if (roi_img_index >= 0 && roi_lng_index > roi_img_index) {
			    start = roi_img_index + lengthOf("serie");
			    roi_serie[roi] = substring(roiset, start, roi_lng_index);
				...
				for(k = 0; k <lengthOf(staining_channel); k++){
					...
					for (object = 0; object < n; object++) {
					    ...
					    roiManager("select", object);
			  			run("Duplicate...", "title="+staining[k]+cell_ID+" duplicate channels="+staining_channel[k]);
			  			roiManager("reset");
			  			roiManager("Add");

						// to get the measurement on all channels
						
						selectWindow(staining[k]+cell_ID);
